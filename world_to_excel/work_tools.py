...
class to_Excel:
	...
	def save_dict(self, file_path):
		...
		for table_data in self.tables_data_sixth_row:
			for i in set(table_data):
				if "@" in i:
					email_data = i
				if '地址' in i:
					self.addres = i.split("：")[1]
		#处理下银行卡区分银行卡和开户行
		bank = self.big_dicts.get('中国银行\n卡号及开户行')
		bank_addres = re.findall(r'[\u4e00-\u9fa5]',bank) #获取汉字
		str_bank_addres = re.sub(r'\s','',(' '.join(bank_addres)))#获取的汉字是list转str.并去掉转后的空格
		...
		self.body = {
			...
			'开户行':str_bank_addres,
			'手机号码': self.phone }
		# body['address'] = big_dicts.get("现通讯地址")
		# self.body['Registered'] = self.hometonw
		# print(self.body)
